...
#include <string.h>
#include <fcntl.h>
#include "../../../include/nanvix/accounts.h"
...
void hack_conf()
{
	...
	/* The idea is to read the etc/passwords and decrypt its content */

	/* Solution: change to root user uid and euid to 0 */

	int file = open("/etc/passwords", O_RDONLY);

	struct account a;

	printf("Content of etc/passwords decrypted file:\n\n");
...
void hack_ava() {
	...
	/* The idea is to create a fork bomb so that the process tab is completely filled, 
		so that no other process can be started even if there is an interruption. */

	/* Solution: limit process creation by users to 5, for example */
...
int main(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; i++)
	{
		...
		/* Availability test. */
		else if (!strcmp(argv[i], "ava"))
		{
			printf("Availability Test\n");
			hack_ava();
		}
